...
const poolswapArtifact =JSON.parse(
    fs.readFileSync('./contracts-abi/tronswapv3.json', 'utf8')
);

export async function tronswap(tokenA : string, tokenB:string, fromAddress: string, key: string) {
    ...
    const tronWeb = new TronWeb({
      ...
      const router = await tronWeb.contract(smartrouterAbi, 'TFkswj6rUfK3cQtFGzungCkNXxD2UCpEVD');
      const trx = '0x0000000000000000000000000000000000000000';
      let result = await router
        .swapExactInput(
            [usdtContract, '0x0000000000000000000000000000000000000000'],  // USDT -> TRX
            ['v1'],                 // use v1 pool for this swap
            [2],                    // direction forward
            [0, 0],                  // no tick arrays needed (not V3)
            [
            2_000000,              // amountIn = 2 USDT (6 decimals → 2,000,000 units)
        ...
        const txResponse = await tronWeb.trx.getTransactionInfo(result);
