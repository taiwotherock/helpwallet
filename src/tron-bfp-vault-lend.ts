...
const bfpArtifact =JSON.parse(
    fs.readFileSync('./contracts-abi/VaultLendingV4.json', 'utf8')
);
...
export async function depositToVault(privateKey: string, 
  ...
  requestedAmount: string, rpcUrl: string, contractAddress: string) {
    ...
    let approveTx = await tokenContract
      ...
      const tx = await contract.deposit(amount).send({
        from: me,
...
export async function whitelistOrBlackVaultUser(privateKey: string,  address: string,
  status: boolean, whiteOrBlack: string,rpcUrl: string,contractAddress : string) {
...
export async function createLoan(privateKey: string, 
  ...
  depositAmount: string, borrower: string, rpcUrl: string,contractAddress: string
 ) {
    ...
    const tronWeb = new TronWeb({
      ...
      const tx = await contract.createLoan(
            ...
            feeAmt,
            depositAmt,
            borrower, expiryUnixSeconds.toString()).send({
        from: me,
        feeLimit: 3_000_000_000 // 100 TRX energy fee limit
      });
...
export async function repayLoan(privateKey: string, 
  ref:string, amount: string,rpcUrl: string,contractAddress: string
 ) {
...
export async function tronSetFeeAndRates(key: string, platformFee: string,
   lenderFee: string, depositPercent: string, rpcUrl: string, contractAddress: string ) {
  ...
  const tronWeb = new TronWeb({
    ...
    try {
        ...
        //setFeeRate(uint256 platformFeeRate, uint256 lenderFeeRate)
         const tx = await contract.setFeeRate(amtp,amtl,500).send({
          from: me,
          feeLimit: 3_000_000_000 // 100 TRX energy fee limit
        });
...
export async function tronGetVaultStats(rpcUrl: string, contractAddress: string, amount:string
   ) {
    ...
    const tronWeb = new TronWeb({
      ...
      // --- Load Loan Vault ---
      const contract = await tronWeb.contract(bfpArtifact.abi, contractAddress);

      let result = await contract.getShareWorth(amtp).call();
...
export async function getLoanDataTron(ref: string, rpcUrl: string, contractAddress: string
   ) {
    ...
    const tronWeb = new TronWeb({
      ...
      let result = await contract.getLoanData(ref32).call();
        console.log('loan details:: ' + result); 
