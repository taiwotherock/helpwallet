...
export async function tronswapv2(tokenA : string, tokenB:string, fromAddress: string, key: string) {
    ...
    const tronWeb = new TronWeb({
      ...
      //await usdt.approve('TFkswj6rUfK3cQtFGzungCkNXxD2UCpEVD', 2_000000).send();
       ...
       const routerAddr = 'TDAQGC5Ekd683GjekSaLzCaeg7jGsGSmbh';  
        ...
        const amountIn     = (2 * 10**6).toString();           
        ...
        console.log(
         ...
         const resultt = await tronWeb.trx.getBalance(me)
...
export async function tronswaptrx(tokenA : string, tokenB:string, fromAddress: string, key: string) {
    ...
    try {
        ...
        // --- Amounts ---
        const amountIn     = (50 * 10 ** 6).toString(); // 100 USDT with 6 decimals
...
async function checkPoolExists(routerContract, token0, token1, fee = 0) {
    try {
        // For V2 pools (no fee parameter)
        if (fee === 0) {
            const pairAddress = await routerContract.getPair(token0, token1).call();
            return pairAddress && pairAddress !== '0x0000000000000000000000000000000000000000';
        } else {
            // For V3 pools (with fee parameter) 
            const poolAddress = await routerContract.getPool(token0, token1, fee).call();
